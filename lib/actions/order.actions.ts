...
import { sendPurchaseReceit } from '@/email';
...
export async function getOrderSummary() {
  ...
  //get monthly sales
  //the line below should be exactly reproduced with single and double quotes where needed
...
export async function deleteOrder(orderId: string) {
  try {
    ...
    return {
      success: false,
      message: formatError(error) || 'An error occurred while deleting order!',
    };
  }
}
...
export async function updateOrderToPaidCOD(orderId: string) {
  try {
    ...
    return {
      success: false,
      message: formatError(error) || 'An error occurred while updating order!',
    };
  }
}
